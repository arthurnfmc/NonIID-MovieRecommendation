...
df = pd.concat([data.drop(columns=['genres', 'occupation', 'zip-code', 'title']), binary_genres_grouped], axis=1)
...
def fold(fold_i):
  ...
  kf = KFold(n_splits=5)
  indexes = kf.split(df)
  for i, (tr, te) in enumerate(indexes):
    if i == fold_i:
      train_index, test_index = tr, te
      break
...
REPORT_F_PATH = f"./results_n_fold={N}-{N_PARTITIONS}_clients-{EXPERIMENT_KIND_PATHSUBSTRING}-{SHARE_PERCENTAGE_PATHSUBSTRING}fed-eval.csv"
with open(REPORT_F_PATH, "a+") as f:
  f.write("fold_i,mae,mse,rmse,precision,recall,ndcg,time_in_seconds\n")
